...
<?php } 
  ...
  $center_code=$_POST['dept']; //dept code
 
  $center_course_code=$_POST['dept_course']; //dept course code
  //echo $center_course_code;
...
and s.belongs_to_center_dept='dept'
ORDER BY s.sl DESC ";
	}
  else {
...
ORDER BY s.sl DESC ";
  ...
  <input type="hidden" name="dept" id="dept" value="<?php echo $center_code;?>">
    <label style="color:red" for="author"><i class="fa fa-search"></i> Course Filter:</label>
   <select onchange="$('#deptSearch').submit();" title="Filter by course" name="dept_course" class="form-control" id="dept_course"  required="true">
                <?php
               
                $query11=mysql_query("select * from course where dept_code=$center_code");?>
...
<option value="0" selected>ALL COURSE</option>
               ...
               {
               ?>
                <option <?php if($center_course_code==$rowdd1['course_id']){echo "selected";} ?> value="<?php echo $rowdd1['course_id']; ?>">
...
<td align="center">
 ...
 <script type="text/javascript">
  ...
  <a data-placement='bottom' rel="tooltip" href="#" title="You cannot directly delete this student from the admin panel. You need to log in to the specific department to delete this student!!"><i  class="fa fa-warning"></i></a>
