...
<?php } ?>
	...
	$dp=$_SESSION['deptcode'];
$bt=$_SESSION['branch_type'];
$course_code=@$_POST['dept_center_course']; 
if(!empty($course_code) || $course_code!=0){?>

<legend><i class="fa fa-users"></i> <?php echo getDepartmentCenter($dp,$bt) ; ?> &raquo; <small style="font-size:12px; font-weight:bold; color:darkgrey"><?php echo getDepartmentCenterCourse($course_code,$bt);?></small></legend>
                                     
                          <?php } else {?>



<legend><i class="fa fa-users"></i> <?php echo getDepartmentCenter($dp,$bt) ; ?>&raquo; <small style="font-size:12px; font-weight:bold; color:darkgrey"> All Students
</small></legend>
...
if($bt=='dept'){
    ...
    $sql="SELECT *
FROM student_details s
WHERE s.dept_center_code =$dp and s.dept_center_course_code=$course_code and belongs_to_center_dept='dept'
ORDER BY s.sl DESC ";
  }
  elseif($bt=='center'){
    ...
    $sql="SELECT *
FROM student_details s
WHERE s.dept_center_code =$dp and s.dept_center_course_code=$course_code and belongs_to_center_dept='center'
ORDER BY s.sl DESC ";
  }
...
$run=mysql_query($sql) or die(mysql_error()) ;?>
    ...
    <form id="deptCenterSearch" action="<?php echo $base;?>studentManager.php" method="post">

  <input type="hidden" name="dept_center_code" id="dept_center_code" value="<?php echo $dp;?>">
    <label style="color:red" for="author"><i class="fa fa-search"></i> Course Filter:</label>
    <?php if($bt=='dept'){?>
   <select onchange="$('#deptCenterSearch').submit();" title="Filter by course" name="dept_center_course" class="form-control" 
   id="dept_center_course"  required="true">
                <?php
               
                $query11=mysql_query("select * from course where dept_code=$dp");?>
               
<?php if(empty($course_code) || $course_code==0){
?>
...
<?php } 
               ...
               {
               ?>
                <option <?php if($course_code==$rowdd1['course_id']){echo "selected";} ?> value="<?php echo $rowdd1['course_id']; ?>">
                <?php  echo $rowdd1['course_name'];?>
                </option>
                <?php 
               } 
                ?>
...
<?php }
                ...
                <?php } elseif($bt=='center'){ ?>

  <select onchange="$('#deptCenterSearch').submit();" title="Filter by course" name="dept_center_course" class="form-control" 
   id="dept_center_course"  required="true">
                <?php
               
                $query11=mysql_query("select * from center_course where center_code=$dp");?>
...
<?php } 
               ...
               {
               ?>
                <option <?php if($course_code==$rowdd1['cc_code']){echo "selected";} ?> value="<?php echo $rowdd1['cc_code']; ?>">
...
<a onClick = "javascript: if (!window.confirm ('Are you sure to delete this student permanently?\n\nPlease note that a student once deleted cannot be restored again into the database.\n\nTherefore, if you are very sure about this, Click OK to proceed, otherwise Click Cancel')) return false;" rel="tooltip"  href="<?php echo $base;?>includes/remove_student_reload.php?id=<?php echo $row['sl'];?>&usertype=<?php echo $_SESSION['user_type'];?>&branch=<?php echo $_SESSION['branch_type'];?>"  title="Click to Delete this student from University Database. Be very sure that you want to delete this student, otherwise all data against this student will be lost permanently" class="label label-danger" id="hghdeletess_<?php echo $row['sl'];?>"><i  class="fa fa-trash-o"></i></a>
