...
foreach (range('A1','C1') as $col) {
	...
	$F->setCellValue('C1', 'DEPARTMENT');
	 

	


$sql="SELECT * from course a,department b where b.dept_code=a.dept_code order by b.dept_name desc";	
...
{   
	...
	$F->setCellValue('B'.$Line, $r['course_name']);
	$F->setCellValue('C'.$Line,$r['dept_name']);
	 $i=$i+1;
	$Line=$Line+1;
}
header('Content-Type: application/vnd.ms-excel');
$filename='DU Department-Course Excel Report-'.time().".xls";
