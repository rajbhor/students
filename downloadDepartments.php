...
foreach (range('A1','C1') as $col) {
	...
	$F->setCellValue('C1', 'DEPARTMENT-CODE');
	 

	


$sql="SELECT * from department order by dept_name desc";	
...
{   
	...
	$F->setCellValue('B'.$Line, $r['dept_name']);
	$F->setCellValue('C'.$Line,$r['sf_code']);
	 $i=$i+1;
	$Line=$Line+1;
}
header('Content-Type: application/vnd.ms-excel');
$filename='DU Department Excel Report-'.time().".xls";
